...
public class GamePanel extends SurfaceView implements Runnable{
    ...
    protected void drawCards(Canvas canvas) {
        ...
        for (int i = 0; i < size; i++) {
            //get bitmap
            Log.d("Card number " + i, cards[i].bmName);
            int id = getResources().getIdentifier(cards[i].bmName, "drawable", context.getPackageName());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                screePressed(initialX, initialY);
    ...
    protected void selectedCard(int index) {
        Log.d("Card was selected", cards[index].name);
    }

    private void screePressed(float x, float y) {
        if (canPress) {
